...
/**
 * @Description
 * redis的key的匹配策略: 若当前的applicationName 与 所对应的 keyBuilder相匹配，则进行加载所需要的keyBuilder类
 */
public class RedisKeyLoadMatch implements Condition {
    ...
    private static final String PREFIX = "yubaolive";

    @Override
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        String appName = context.getEnvironment().getProperty("spring.application.name");
        if (appName == null) {
            LOGGER.error("[RedisKeyLoadMatch]没有匹配到应用名称，所以无法加载任何RedisKeyBuilder对象");
            return false;
        }
        try {
            ...
            LOGGER.info("[RedisKeyLoadMatch]keyBuilderClass is {},matchStatus is {}", keyBuilderName, matchStatus);
