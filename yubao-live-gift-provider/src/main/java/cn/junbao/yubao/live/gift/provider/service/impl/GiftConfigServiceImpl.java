...
import cn.junbao.yubao.live.gift.provider.dao.mapper.IGiftConfigMapper;
...
public class GiftConfigServiceImpl implements IGiftConfigService {
    ...
    @Resource
    private IGiftConfigMapper giftConfigMapper;
    @Resource
    ...
    public GiftConfigDTO getByGiftId(Integer giftId) {
        ...
        GiftConfigPO giftConfigPOReq = new GiftConfigPO();
        giftConfigPOReq.setGiftId(giftId);
        giftConfigPOReq.setStatus(CommonStatusEum.VALID_STATUS.getCode());
        GiftConfigPO giftConfigPO = giftConfigMapper.getGiftConfigById(giftConfigPOReq);
        //检索出来的数据，要重新存入cache中
    ...
    public List<GiftConfigDTO> queryGiftList() {
        ...
        //list集合去进行存放
        List<GiftConfigPO> giftConfigPOList = giftConfigMapper.selectList();
    ...
    public void insertOne(GiftConfigDTO giftConfigDTO) {
        ...
        giftConfigMapper.insertOne(giftConfigPO);

        redisTemplate.delete(cacheKeyBuilder.buildGiftListCacheKey());
        redisTemplate.delete(cacheKeyBuilder.buildGiftConfigCacheKey(giftConfigPO.getGiftId()));
    }

    @Override
    public void updateOne(GiftConfigDTO giftConfigDTO) {
        GiftConfigPO giftConfigPO = ConvertBeanUtils.convert(giftConfigDTO, GiftConfigPO.class);
        giftConfigMapper.updateOne(giftConfigPO);
        redisTemplate.delete(cacheKeyBuilder.buildGiftListCacheKey());
